...
WHERE membercost =0
LIMIT 0 , 30
There are 4 facilities that don't charge a fee to members
...
the member name. */
SELECT b.bookid, b.facid, b.memid, f.name AS court, (m.firstname + ' ' + m.surname) AS full_name FROM `Bookings` AS b
...
SELECT CONCAT( m.firstname, " ", m.surname ) AS name, f.name AS facility_name, starttime
FROM Bookings b
JOIN Members m ON b.memid = m.memid
JOIN Facilities f ON b.facid = f.facid
WHERE starttime LIKE '%2012-09-14%'
...
/* Q11: Produce a report of members and who recommended them in alphabetic surname,firstname order */
        SELECT A.firstname ||" "|| A.surname AS member_name, B.firstname ||" "|| B.surname AS referral
        FROM Members A, Members B
        WHERE A.memid = B.recommendedby
        ORDER BY A.surname, A.firstname

/* Q12: Find the facilities with their usage by member, but not guests */
        SELECT f.name AS facility, COUNT(b.facid) AS usage
        FROM Facilities f
...
/* Q13: Find the facilities usage by month, but not guests */
        SELECT f.name AS facility, COUNT(b.facid) AS usage, strftime('%m', b.starttime) AS month
        FROM Facilities f
        INNER JOIN Bookings b
        USING(facid)
        INNER JOIN Members m
